...
def generate_audio(
    ...
    *,
    ...
    if prompt_id is None:
        ...
        print("[generate_audio] prompt_id is required", flush=True)
        return None
    ...
    try:
        ...
        if not ref or not getattr(ref, "file_path", None):
            ...
            print(f"[generate_audio] no audio found for course_id `{course_id}` and slot {slot}", flush=True)
            ...
            if not ref_path.is_file():
                ...
                print(f"[generate_audio] DB voice not found on disk: {ref_path}", flush=True)
                ref_path = Path("/app/database/voice_sample/krusche_voice_v2.mp3")
        ...
        print(f"[generate_audio] {prompt_id}#{slot}: Posting to {audio_api_url} with {ref_path}", flush=True)
        with ref_path.open("rb") as f:
            ...
            if "application/json" in (resp.headers.get("Content-Type") or "").lower():
                try:
                    ...
                    print(f"[generate_audio] Unexpected JSON response: {resp.json()}", flush=True)
                except Exception as exception:
                    logger.error("[generate_audio] Unexpected JSON response (could not parse)", exc_info=exception)
                    print(
                        f"[generate_audio] Unexpected JSON response (could not parse): {exception!r}",
                        flush=True,
                    )
                return None
        ...
        if not tmp_path.exists() or tmp_path.stat().st_size == 0:
            ...
            print("[generate_audio] Empty file received", flush=True)
            tmp_path.unlink(missing_ok=True)
            return None

        tmp_path.replace(wav_path)
        logger.info(f"[generate_audio] OK -> {wav_path}")
        print(f"[generate_audio] OK -> {wav_path}", flush=True)
        return str(wav_path)

    except requests.RequestException as exception:
        logger.error("[generate_audio] Request error", exc_info=exception)
        print(f"[generate_audio] Request error: {exception!r}", flush=True)
        return None
    except Exception as exception:
        logger.error("[generate_audio] Unexpected error", exc_info=exception)
        print(f"[generate_audio] Unexpected error: {exception!r}", flush=True)
        return None
...
def generate_video(
    ...
    if prompt_id is None or video_counter is None:
        ...
        print("[generate_video] prompt_id and video_counter are required", flush=True)
        return None
    ...
    if not Path(resolved_audio).is_file():
        ...
        print(f"[generate_video] Audio file not found: {resolved_audio}", flush=True)
        return None
    ...
    if not source_path or not Path(source_path).is_file():
        ...
        print(
            f"[generate_video] Source image not found: {source_path}, using fallback sample",
            flush=True,
        )
    ...
    if not Path(source_path).is_file():
        ...
        print(f"[generate_video] Source image not found: {source_path}", flush=True)
        return None
    ...
    try:
        ...
        print(
            f"[generate_video] Posting to {video_api_url} with {resolved_audio} and {source_path}",
            flush=True,
        )
        with (
            ...
            if resp.status_code >= 400:
                ...
                print(f"[generate_video] HTTP {resp.status_code}: {resp.text[:200]}", flush=True)
                return None
        ...
        if temp_path.stat().st_size == 0:
            ...
            print("[generate_video] Empty file received", flush=True)
            temp_path.unlink(missing_ok=True)
            return None

        temp_path.replace(final_path)
        logger.debug(f"[generate_video] OK -> {final_path}")
        print(f"[generate_video] OK -> {final_path}", flush=True)
        return str(final_path)

    except requests.RequestException as exception:
        logger.error("[generate_video] Request error", exc_info=exception)
        print(f"[generate_video] Request error: {exception!r}", flush=True)
        return None
    except Exception as exception:
        logger.error("[generate_video] Unexpected error", exc_info=exception)
        print(f"[generate_video] Unexpected error: {exception!r}", flush=True)
        return None
