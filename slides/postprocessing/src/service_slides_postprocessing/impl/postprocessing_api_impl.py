...
_log = logging.getLogger("postprocessing_impl")
...
class ImplPostprocessingApi(BasePostprocessingApi):
    ...
    async def store_slideset(
        ...
        _log.info("Storing slideset %s", store_slideset_request.slideset.prompt_id)
        ...
        _log.debug("Building slideset %s with theme %s", slideset.prompt_id, theme_path)

        web_available = slidev_helper.build_web_distribution(
        ...
        if not web_available:
            _log.error("Slideset could not be built. View will not be available.")

        for asset in slideset.assets:
            asset_file = path_helper.get_web_asset_file(slideset.prompt_id, asset.path)
            pathlib.Path(asset_file).parent.mkdir(parents=True, exist_ok=True)
            with open(asset_file, "b+w") as f:
                f.write(b64decode(asset.data))

        # Step 3: Export PDF
        # TODO: Currently disabled, because it produces missing dependency errors. This has to be fixed with some work.
        #prefix_slide = "---\nlayout: fact\n---\n\n# These slides are AI generated by Orpheus / Diese Folien wurden von Orpheus durch AI erstellt\n\n"
        ...
        #    f.write(slideset.slideset)
#
        #pdf_available = slidev_helper.export_pdf(
        #    work_dir=path_helper.get_markdown_directory(slideset.prompt_id),
        #    source_path=pdf_source_path,
        #    output_file=path_helper.get_export_path(slideset.prompt_id),
