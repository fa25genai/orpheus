...
class StoreSlidesetRequest(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["theme", "slideset"]

    @field_validator("theme")
    def theme_validate_enum(cls: Any, value: str | None) -> str | None:
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(["default", "tum"]):
            raise ValueError("must be one of enum values ('default', 'tum')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of StoreSlidesetRequest from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of slideset
        if self.slideset:
            _dict["slideset"] = self.slideset.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of StoreSlidesetRequest from a dict"""
        if obj is None:
            return None
